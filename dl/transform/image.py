...
class RandomTransformation:
    ...
    def _get_random_values(self, x):
        # We increase the translation for bigger images
        ...
        translation = tensor_utils.tensor(translation, dtype=x.dtype, device=x.device)
        scale = tensor_utils.tensor(scale, dtype=x.dtype, device=x.device)
        angle = tensor_utils.tensor(angle, dtype=x.dtype, device=x.device)
